---
name: use-gh
description: Manages GitHub pull requests, issues, and CI runs using the gh CLI. Use when creating PRs, reviewing, merging, posting comments, checking CI status, or any gh operation. Do NOT use for GitLab repositories (use use-glab).
---
...
- **Always squash merge** and **delete the source branch** after merge — squash merges maintain linear history making bisect and revert trivial, and deleting branches prevents stale branch accumulation
- **Detect PR templates** before creating a PR — check for `.github/pull_request_template.md` or templates in `.github/PULL_REQUEST_TEMPLATE/`. A blank body ignores required context fields (testing plan, reviewer checklist) that the team relies on
- **Inline review comments require `gh api`** — `gh pr comment` only posts top-level comments
- **Confirm before mutations** — don't create, merge, or comment without user approval because these are remote state changes that are difficult or impossible to reverse cleanly
...
If not found: install via system package manager or https://cli.github.com.
...
Add `--auto` to merge automatically once CI passes.
...
- PR created successfully on GitHub with correct title, body, and base branch
- PR template detected and applied when present in the repository
- Squash merge completed and source branch deleted
- CI status retrieved and reported to the user
- Inline review comments posted on correct file and line
- User confirmation obtained before every mutation (create, merge, comment)
