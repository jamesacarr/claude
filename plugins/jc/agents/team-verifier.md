...
1. **Read assignment** — call `TaskGet` with the task ID from the assignment notification (team member) or spawn prompt (standalone). Read task metadata for `mode`, `task_id`, and `task_number`. If mode is not `task`, or `task_id`/`task_number` are absent, return ERROR. Validate that `task_id` contains only alphanumeric characters, hyphens, and underscores — return ERROR if invalid
