...
1. Spawn 4 mappers via `Agent(subagent_type: "jc:team-mapper", team_name: "{team-name}", name: "mapper-{focus}", description: "Map {focus} dimension", prompt: "You are mapper-{focus} for team {epic-id}-refinement. You will be notified when your task is assigned.")`
...
1. Spawn 4 shapers via `Agent(subagent_type: "jc:team-refinement-panelist", team_name: "{team-name}", name: "shaper-{persona-slug}", description: "Shape as {persona-slug}", prompt: "You are shaper-{persona-slug} for team {epic-id}-refinement. You will be notified when your task is assigned.")`
...
1. Spawn via `Agent(subagent_type: "jc:team-refinement-panelist", team_name: "{team-name}", name: "shaper-tech-debt-scout", description: "Shape as tech-debt-scout", prompt: "You are shaper-tech-debt-scout for team {epic-id}-refinement. You will be notified when your task is assigned.")`. Create task via `TaskCreate` with metadata `{"persona": "Tech Debt Scout", "epic_id": "<epic-id>", "codebase_map_dir": ".planning/codebase/"}`. Assign via `TaskUpdate(owner: "shaper-tech-debt-scout")`
...
4. Create the refinement team via `TeamCreate(team_name: "{epic-id}-refinement")` — capture the returned `team_name` as `{team-name}` (see ASSESS step 4)
